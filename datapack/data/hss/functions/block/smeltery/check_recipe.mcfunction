...
execute if score @s[type=item,nbt={Item:{id:"minecraft:lava_bucket"}}] count matches 1.. if score @e[type=item,distance=..1,nbt={Item:{id:"minecraft:water_bucket"}},sort=nearest,limit=1] count matches 1.. run function hss:recipes/smeltery/obsidian_from_lava_bucket
