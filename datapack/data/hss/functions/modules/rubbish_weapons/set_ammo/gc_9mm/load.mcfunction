...
execute as @e[type=minecraft:armor_stand,tag=master,limit=1] at @s run data modify block ~ ~ ~ Text1 set value '[{"translate":"gui.hss.bullets","color":"gold","italic":false},{"text":": ","color":"white","italic":false},{"translate":"%s ","with":[{"score":{"name":"player_9mm_count_ammo","objective":"count"}}],"color":"green","italic":false}]'
...
execute as @e[tag=player_9mm_count_ammo,distance=..1.5,limit=1] run data modify entity @s ArmorItems[0].tag.display.Name set from entity @e[type=minecraft:armor_stand,tag=master,limit=1] ArmorItems[0].tag.display.Name
