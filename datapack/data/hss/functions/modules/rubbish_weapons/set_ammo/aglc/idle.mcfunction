...
execute as @e[type=minecraft:player,scores={debug=1}] run tellraw @s "[§b§lMODULE§f] Module§9 hss:modules/aglc/idle §floaded."

execute store result score player_sniper_count_ammo count run scoreboard players get @s sniperAmmo 
execute align xyz run summon minecraft:armor_stand ~ ~ ~ {Small:1b,Marker:1b,Invulnerable:1b,Invisible:1b,Silent:1b,NoGravity:1b,ArmorItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{rubbishGuns:1b,aglc:1b,isEmpty:0b,munition:0b,durability:0b,CustomModelData:4,display:{Name:'[{"translate":"item.hss.aglc","color":"white","italic":false}]'}}},{},{},{}],Tags:["player_sniper_count_ammo"]}
