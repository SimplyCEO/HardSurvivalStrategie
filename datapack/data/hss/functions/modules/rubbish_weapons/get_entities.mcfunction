...
#execute as @s[nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{rubbishGuns:1b}}}] unless score @s isRunning matches 1.. unless block ~ ~-.1 ~ air run function hss:modules/rubbish_weapons/give_scores
#execute if score @s loadedGun matches 0 unless score @s isRunning matches 1.. unless block ~ ~-.1 ~ air run function hss:modules/rubbish_weapons/detect_load
